...
**Scoped exception — Three.js / React Three Fiber.** `three`, `@react-three/fiber`, and `@react-three/drei` are approved for **exactly one signature 3D concept: the Speakers section's mystery-figure reveal** — rendered per-card (`components/sections/SpeakerCard.tsx` + `components/three/SpeakerCardFigure.tsx`), one small independent canvas per slot rather than a single hero-scale scene, since the section no longer has a standalone 3D centerpiece. This is still one concept, not nine: same model, same rim-lit/silhouetted treatment, same reason for existing (WebGL particles/lighting Framer Motion can't fill) — not a license for 3D elsewhere on the site. Requirements for this exception to stay valid:
